...
`FromEnv`; `LLMWIRE_EMULATE_OPENCODE=true` is the operator's switch for a host
the profiles do not mark (it adds the identity, never removes it), and a client
built with an explicit `Config.BaseURL` can set `EmulateOpenCode: true` by hand. Headers only, never a request body. The session id is the client's own — minted at construction,

...
| `reasoning.can_be_disabled` | `ZaiThinkingCanBeDisabled` (pattern; hardcoded to glm) | copy the probe for the new model, sending its control's OFF shape: `thinking:{"type":"disabled"}` for `toggle_object`, `reasoning_effort: none` for `effort` | REJECTED → `false`; ACCEPTED with reasoning tokens 0 → `true` | MiMo's `true` is doc-derived today: no probe has sent the toggle there. `enabled_by_default` is a SEPARATE bool, read it off a request with no knob at all |
